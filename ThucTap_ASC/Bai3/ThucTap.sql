...
select COUNT(*) from TBLHuongDan 
--2.	In ra màn hình danh sách HoTen sinh viên
select Hotensv from TBLSinhVien
--3.	Thêm một sinh viên tên: Trần Nam Dương/2018/Geo/1995/Ho Chi Minh
select * from TBLSinhVien
insert into TBLSinhVien values(9,'Trần Nam Dương','Geo','1995','Ho Chi Minh')
delete TBLSinhVien where Masv=9
--4.	Cập nhật sinh viên Le Thi Van năm sinh 2018, Quê quán Ha nam
update TBLSinhVien set Namsinh=222,Quequan='ádasd' where Hotensv='Le Thi Van'
--5.	Xóa đề tài Dt04
delete TBLDeTai where Madt='Dt04'
select * from TBLDeTai

select * from TBLHuongDan
--B. Chỉ sử dụng 01 lệnh SQL trả lời các yêu cầu sau:
--I.
--1.	Đưa ra thông tin gồm mã số, họ tênvà tên khoa của tất cả các giảng viên 72239
select Magv,Hotengv,TBLKhoa.Tenkhoa
from TBLGiangVien,TBLKhoa 
where TBLGiangVien.Makhoa=TBLKhoa.Makhoa
--cách 1  32875
create table #taolocal_gv
(
 Magv int,
 Hotengv char(30),
)
insert into #taolocal_gv select Magv,Hotengv from TBLGiangVien
select * from #taolocal_gv
drop table #taolocal_gv
-- cách 2 -- 32875
select Magv,Hotengv,TBLKhoa.Tenkhoa
into #taolocal_giangvien
from TBLGiangVien,TBLKhoa
where TBLGiangVien.Makhoa=TBLKhoa.Makhoa
select * from #taolocal_giangvien

drop table #taolocal_giangvien
--tao bảng tam global
--cách 1
create table ##taolocal_gv
(
...
--cách 2
select Magv,Hotengv,TBLKhoa.Tenkhoa
into ##taolocal_giangvienglobalcanh2
from TBLGiangVien,TBLKhoa
where TBLGiangVien.Makhoa=TBLKhoa.Makhoa
select * from ##taolocal_giangvienglobalcanh2
drop table ##taolocal_giangvienglobalcanh2
...
select * from TBLSinhVien sv inner join TBLHuongDan hd on sv.Masv= hd.Masv 
where hd.KetQua=0 or hd.KetQua is  null
select dt.Madt,SoLuongSinhVien=COUNT(hd.Masv) from TBLDeTai dt inner join TBLHuongDan hd on dt.Madt=hd.Madt
inner join TBLSinhVien sv on hd.Masv=sv.Masv group by dt.Madt

select sv.Makhoa,sv.Masv,sv.Hotensv,gv.Magv,gv.Hotengv,dt.Madt,dt.Tendt,hd.KetQua from TBLHuongDan hd inner join TBLSinhVien sv on hd.Masv=sv.Masv
inner join TBLGiangVien gv on hd.Magv=gv.Magv inner join TBLDeTai dt on hd.Madt=dt.Madt
